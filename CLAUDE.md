...
兩段都吃 `data/<board>/` 任一板；驗證方式由各板 `board.yaml` 決定。
...
2. **領域知識零寫死**——腳位/訊號/AF/周邊知識一律來自 `data/<board>/`
   ...
   第二段是 `patch_agent/config.py`（一律 import config 常數；多板由
   `init_board()` 重算，只准在 CLI 入口與 web single-flight DTS 工作者呼叫）
   ——兩邊都**不得自行拼路徑字串**。`board.yaml` 是**策略檔**（vendor、
   驗證方式），**不是第三個路徑權威**——它永不描述知識庫內部佈局。
...
- **板級驗證策略**（`data/<board>/board.yaml` 的 `validation` 區塊）：
  ...
  非錯誤）。現階段除 stm32mp257f-ev1 外一律 `enabled: false`。
